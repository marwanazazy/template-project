...
def scoring_system(score):
    ...
    """
    if score >= 20:
        return f"Congratulations! You helped {character_name} navigate the woods successfully."
    elif score <= 15:
        return f"{character_name} made it back home with some scratches, but learned valuable lessons."
    else:
        return f"Unfortunately, {character_name} didn't make it back home safely. Better luck next time!"
...
def adventure_story(turn_limit=2):
    ...
    if choice == "1":
        ...
        print_delay(f"{character_name} pushed open the creaky door of the cabin "
                    ...
                    f"map on a table. The map showed a path leading out of the "
                    f"woods. Relieved, {character_name} pocketed the map and "
    ...
    # Check if the turn limit is reached
    if turns >= turn_limit:
        print(f"Turn limit of {turn_limit} reached.")

    # If turn limit not reached, continue the adventure
